...
class UpdatePlugin(Screen, ProtectedScreen):
	...
	def checkTraficLight(self):
			...
			try:
				...
				if isinstance(message, dict):
					...
					if lang in message:
						...
						message = _("The current image might not be stable.\nFor more information see %s.") % ("https://areadeltasat.net")
		...
		# or display a generic warning if fetching failed
			...
			message = _("The status of the current image could not be checked because %s can not be reached.") % ("https://download.areadeltasat.net")
